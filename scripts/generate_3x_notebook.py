...
def create_3x_notebook():
    ...
    # -------------------------------------------------------------
    c1 = """# Comparative SISR Training Pipeline on Kaggle (Dual T4 GPUs) — Scale 3x (10 Epochs)
## Models: SRCNN, ESPCN, FSRCNN, VDSR, EDSR on Medical X-Ray Dataset

This notebook provides an automated, multi-GPU training and evaluation pipeline for **5 classic Super-Resolution models at 3x scale factor**, optimized for 10 epochs:
1. **SRCNN** (*Dong et al., ECCV 2014*): 3-layer CNN with Bicubic pre-upsampling ($\times 3$).
2. **ESPCN** (*Shi et al., CVPR 2016*): Post-upsampling with Sub-Pixel Convolution (PixelShuffle $\times 3$).
3. **FSRCNN** (*Dong et al., ECCV 2016*): Fast SRCNN with hourglass mapping and deconvolution (stride=3, output_padding=2).
4. **VDSR** (*Kim et al., CVPR 2016*): 20-layer Deep CNN with Global Residual Learning and gradient clipping.
5. **EDSR** (*Lim et al., CVPRW 2017*): Enhanced Deep Residual Network without BatchNorm (PixelShuffle $\times 3$).

### Key Optimizations for 3x Scale:
- **Fast Training (10 Epochs)**: Balances rapid execution on Kaggle while reaching solid convergence.
- **Divisible Patch Sizing (144x144)**: HR patches ($144\times 144$) map cleanly to LR inputs ($48\times 48$) without sub-pixel rounding errors ($144 / 3 = 48$).
- **Dual T4 GPU Support**: Automatic `nn.DataParallel` distribution across 2x NVIDIA T4 GPUs.
- **Stabilized Gradient Clipping**: Added gradient clipping for both VDSR and FSRCNN to prevent divergence.
- **Standalone Clean Weights**: Automatically strips `module.` prefixes so weights can be loaded anywhere (single GPU, CPU, or embedded platforms).
- **One-Click ZIP Export**: Automatically archives all trained `_3x.pth` models into `sr_models_weights_3x.zip` in `/kaggle/working/`."""
...
WEIGHTS_DIR = os.path.join(WORKING_DIR, "weights_3x")
...
print(f"[INFO] Output Weights Directory: {WEIGHTS_DIR}")"""
    ...
    # -------------------------------------------------------------
    # Cell 3: Configuration for 3x Scale & 10 Epochs
    # -------------------------------------------------------------
    c3 = """# ==========================================
# HYPERPARAMETERS & CONFIGURATION (SCALE 3X)
# ==========================================
UPSCALE_FACTOR = 3           # Scaling factor: 3x
CROP_SIZE = 144              # High-Resolution (HR) patch size (144 / 3 = 48 clean integer)
BATCH_SIZE = 32 if NUM_GPUS >= 2 else 16
NUM_EPOCHS = 10              # 10 epochs for efficient training
...
class MedicalSRDataset(Dataset):
    \"\"\"
    PyTorch Dataset generating paired (LR, HR) tensors for 3x super-resolution.
    HR is randomly cropped (144x144) and augmented, LR is generated via Bicubic downsampling (48x48).
    \"\"\"
    def __init__(self, image_paths, crop_size=144, upscale_factor=3, is_train=True):
...
def calculate_ssim(img1, img2, window_size=11):
    ...
    # -------------------------------------------------------------
    # Cell 6: Model Architectures Adapted for 3x Scale
    # -------------------------------------------------------------
    c6 = """# ==========================================
# 5 SUPER-RESOLUTION MODEL ARCHITECTURES (SCALE 3X)
# ==========================================
...
class ESPCN(nn.Module):
    def __init__(self, in_channels=3, upscale_factor=3):
        ...
        self.tanh2 = nn.Tanh()
        # For 3x: in_channels * (3^2) = 3 * 9 = 27 channels
...
class FSRCNN(nn.Module):
    def __init__(self, in_channels=3, upscale_factor=3, d=56, s=12, m=4):
        ...
        # Deconvolution for 3x: stride=3, padding=4, output_padding=2
...
class EDSR(nn.Module):
    def __init__(self, in_channels=3, upscale_factor=3, n_feats=64, n_resblocks=8, res_scale=0.1):
        ...
        # For 3x: n_feats * (3^2) = 64 * 9 = 576 channels
...
class SRGAN(nn.Module):
    \"\"\"Canonical SRGAN Generator with 16 Residual Blocks for 3x upsampling.\"\"\"
...
def build_model(model_name, in_channels=3, upscale_factor=3):
    \"\"\"Factory function to instantiate models for scale 3x.\"\"\"
...
print("===== 3X MODEL PARAMETER COUNTS =====")
for m_name in ["SRCNN", "ESPCN", "FSRCNN", "VDSR", "EDSR", "SRGAN"]:
    ...
    # -------------------------------------------------------------
    # Cell 7: Multi-GPU Training & Validation Engines
    # -------------------------------------------------------------
    c7 = """# ==========================================
...
def evaluate_epoch(model, dataloader, criterion, device):
    ...
    # -------------------------------------------------------------
    # Cell 8: Main Training Pipeline (10 Epochs, Dual GPU Support)
    # -------------------------------------------------------------
    c8 = """# ==========================================================
...
for model_name in MODELS_TO_TRAIN:
    ...
    # -------------------------------------------------------------
    c9 = """# ==========================================
# PRELIMINARY BENCHMARK SUMMARY TABLE (3X)
# ==========================================
df_summary = pd.DataFrame(summary_records)
print("\\n=======================================================")
print("  PRELIMINARY EVALUATION SUMMARY (PSNR & SSIM - SCALE 3X)")
print("=======================================================")
display(df_summary)
...
print(f"\\n[INFO] Saved benchmark summary table to: {csv_summary_path}\")"""
    ...
    # -------------------------------------------------------------
    c10 = """# ==========================================
# TRAINING CURVES (LOSS, PSNR, SSIM - SCALE 3X)
# ==========================================
...
print(f"[INFO] Saved training curves to: {curves_plot_path}")"""
    ...
    # -------------------------------------------------------------
    # Cell 11: Visual Comparison on Sample Image
    # -------------------------------------------------------------
    c11 = """# ==========================================================
# VISUAL INFERENCE: BICUBIC vs SRCNN vs ESPCN vs FSRCNN vs VDSR vs EDSR vs HR (3X)
# ==========================================================
...
print(f"[INFO] Saved visual comparison to: {visual_comp_path}")"""
    ...
    # -------------------------------------------------------------
    # Cell 12: One-Click ZIP Export for Kaggle
    # -------------------------------------------------------------
    c12 = """# ==========================================================
# ARCHIVE & DOWNLOAD 3X WEIGHTS (KAGGLE ONE-CLICK EXPORT)
# ==========================================================
zip_output_path = os.path.join(WORKING_DIR, f"sr_models_weights_{UPSCALE_FACTOR}x.zip")

print(f"[INFO] Packaging all 3x weights from '{WEIGHTS_DIR}' into ZIP...")
...
print(f"[SUCCESS] 3x Weights Archive ready: {zip_output_path} ({zip_size_mb:.2f} MB)")
print("👉 You can now download this ZIP file directly from the Kaggle Output tab in the right sidebar!")"""
    add_cell("code", c12)

    # Write notebook file
    target_path = "train_models_3x.ipynb"
