...
While sub-pixel convolution (PixelShuffle) eliminates deconvolutional checkerboard artifacts and reduces theoretical arithmetic operations by operating on the low-resolution grid, it introduces substantial control and memory overhead into streaming Register-Transfer Level (RTL) pipelines. Specifically, PixelShuffle mandates complex multi-phase channel demultiplexing, multi-bank memory shuffling, and asynchronous multi-rate clock domain crossings between the Low-Resolution feature extraction core and the High-Resolution output interface. For deep, compute-heavy networks comprising dozens of cascading layers (such as RCAN or EDSR), the $r^2$ arithmetic reduction easily justifies this architectural overhead. Conversely, for an ultra-compact three-layer mapping network like our hardware-tailored SRCNN variant ($1 \rightarrow 16 \rightarrow 8 \rightarrow 1$, totaling only 1,649 parameters), the absolute computational volume is already minimal (requiring only 1,624~MACs per output pixel, which amounts to merely 26.61~MMACs or $<0.027$~GMACs per $128 \times 128$ patch). Even for the standard baseline SRCNN \cite{dong2014learning} with 8,032 parameters, the total computational volume remains modest ($\approx 0.13$~GMACs per patch). In this ultra-lightweight regime, the hardware area, routing congestion, and latency penalty of spatial re-ordering logic far outweigh any arithmetic power savings. By combining a single-rate streaming bicubic pre-interpolator with dimensionally uniform convolution stages, our pre-upsampling pipeline guarantees 100\% Multiply-Accumulate (MAC) efficiency (one output pixel per clock cycle) with strictly feed-forward line buffering and zero pipeline stall overhead.
...
\hline
Kim \textit{et al.} \cite{kim2019real} (2019) & Xilinx Virtex-7 & 740 & 12.80 W & 60.0 fps (4K) & Fixed 16-bit & Natural Video / SR \\
Mao \textit{et al.} \cite{mao2020fdna} (2020) & Xilinx Kintex-7 & 1512 & 5.40 W & 62.7 fps (UHD) & Fixed 13-bit & Natural Images / F-DNA (FSRCNN) \\
Liu \textit{et al.} \cite{liu2024high} (2024) & Xilinx ZCU102 & 1210 & 16.85 W & 35.8 fps (FHD) & Fixed 8-bit & Natural Images / Edge SR \\
Chang \textit{et al.} \cite{chang2024hdsuper} (2024) & Xilinx XC7Z035 & 704 & 2.08 W & 81.0 fps (FHD) & Fixed 16-bit & Natural Images / HDSuper (LSR) \\
...
% Page break to cleanly isolate standalone bibliography and avoid column bleeding (commented out to eliminate blank trailing whitespace)
% \newpage
