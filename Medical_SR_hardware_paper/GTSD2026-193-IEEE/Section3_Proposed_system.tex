...
\caption{Hardware fixed-point arithmetic datapath and binary point alignment mechanism: (a) Input pixel conversion to signed S7.0, (b) Convolutional weight quantization into fractional S0.7, (c) Signed multiplication yielding 16-bit intermediate products ($\text{S8.7}$), (d) Left bit-shifting of bias ($\ll 7$) for binary point synchronization before 24-bit accumulation ($\text{S16.7}$), and (e) Arithmetic right-shifting ($\gg 7$) with ReLU and saturation clamping.}
...
A critical mathematical consideration in fixed-point arithmetic is the alignment of fractional radix points during Multiply-Accumulate (MAC) operations. Multiplying a signed activation $X \in \text{S7.0}$ ($X \in [-128,\, +127]$) by a signed weight $W_q \in \text{S0.7}$ ($W_q \in [-128,\, +127]$, representing real coefficients within $[-1.0,\, +0.9921875]$) produces an intermediate product $P = X \cdot W_q$ with $7 + 0 = 7$ fractional bits. In signed two's-complement arithmetic, multiplying two 8-bit signed integers spans from $(-128) \times (+127) = -16,256$ up to the asymmetric corner case $(-128) \times (-128) = +16,384$. Because $+16,384$ strictly exceeds the maximum positive capacity of a 15-bit signed integer ($+16,383 = 2^{14} - 1$), the worst-case product structurally requires a full 16-bit signed representation ($[-32,768,\, +32,767]$). In the physical RTL implementation, this product is registered in a \textbf{16-bit signed $\text{S8.7}$ format} ($1\text{ sign bit} + 8\text{ integer bits} + 7\text{ fractional bits} = 16\text{ bits}$), completely preventing arithmetic overflow and natively matching standard FPGA DSP multiplier output ports.

Because layer biases $B_k$ share the activation dynamic range and are natively quantized in S7.0 format, they cannot be directly added to the product sum without inducing catastrophic scale misalignment. To synchronize binary points before accumulation, each bias term $B_k$ is bit-shifted left by 7 positions ($B_k \ll 7 = B_k \cdot 2^7$), lifting its representation into the coordinate space with 7 fractional bits. The unified MAC accumulation for output channel $k$ at coordinate $(i, j)$ is formulated as:
\begin{equation}
...
\begin{equation}
    \text{WNS} = +0.903\text{ ns}, \quad \text{WHS} = +0.045\text{ ns} \quad (\text{at } 100\text{ MHz}),
    \label{eq:wns}
\end{equation}
...
Worst Hold Slack (WHS) & +0.045~ns & --- & Timing Met \\
