...
\begin{equation}
M_{min} = \left\lceil \frac{R - 1}{2} \right\rceil = \left\lceil \frac{13 - 1}{2} \right\rceil = 6\text{ pixels}
\label{eq:margin}
\end{equation}
...
For a tile of width $W = 128$, our design employs a cascading shift-register architecture implemented with dual-port Block RAMs (BRAM36K configured as dual-port 18Kb/36Kb FIFOs) and distributed LUTRAM:
\begin{itemize}
...
Because activations are formatted as signed 8-bit integers ($\text{S7.0}$) and weights as signed 8-bit fractions ($\text{S0.7}$), the multiplication operands map into the low-order bits of the DSP48E1 inputs without precision loss or dynamic range clipping. Furthermore, all 1,624 model weights and 25 biases are pre-loaded into distributed on-chip ROM, eliminating weight fetch latency during run-time inference.

To achieve an optimal tradeoff between computational throughput and hardware resource limits, the arithmetic datapath allocates exactly \textbf{120 DSP48E1 slices} (54.55\% of the 220 available on the Zynq-7020):
\begin{itemize}
    \item \textbf{Layer~1 (Feature Extraction):} Evaluates 16 parallel output filters across the $9 \times 9$ spatial window. The spatial products are computed across parallel DSP engines and aggregated via balanced adder trees.
    \item \textbf{Layer~2 (Non-linear Mapping):} Processes 8 output channels from 16 input channels using 128 parallel product accumulations.
    \item \textbf{Layer~3 (Reconstruction):} Convolves 8 input channels across a $5 \times 5$ kernel to synthesize the single-channel high-resolution output pixel.
\end{itemize}
...
\begin{equation}
    \text{WNS} = +0.903\text{ ns} \quad (\text{at } 100\text{ MHz}),
    \label{eq:wns}
\end{equation}
confirming reliable high-speed operation with a 9\% timing margin.

The accelerator delivers a sustained initiation interval of $II = 1$, producing one output pixel per clock cycle once the pipeline is primed. The initial pipeline ramp-up latency ($L_{ramp}$) is dominated by the time required to fill the first 8 line buffers of Layer~1:
\begin{equation}
    L_{ramp} = (K_1 - 1) \cdot W + L_{pipe} = 8 \cdot 128 + 26 = 1,050\text{ clock cycles},
    \label{eq:ramp}
\end{equation}
where $L_{pipe}$ represents the internal register stages of the line buffers, MAC units, and adder trees. Processing an entire $128 \times 128$ tile requires:
\begin{equation}
...
When integrated with host AXI DMA transfers, cache coherency management, and CPU overlap-tiling coordination, the total measured wall-clock execution time is \textbf{6.33~ms per $128 \times 128$ tile}, providing a sustained system throughput of \textbf{157.9~patches/s}. Furthermore, the PL dynamic logic power dissipation is measured at a mere \textbf{0.047~W} (with total Zynq-7020 SoC power of 1.438~W), confirming outstanding energy efficiency for clinical edge devices. A comprehensive summary of the accelerator's microarchitectural specifications and resource consumption is presented in Table~\ref{tab:accelerator_specs}.

\begin{table}[t]
\caption{Microarchitectural Specifications and Resource Utilization of the Proposed Accelerator on Xilinx Zynq-7020}
...
BRAM36K Blocks & 3.5 & 140 & 2.50\% \\
Slice LUTs & 14,280 & 53,200 & 26.84\% \\
Slice Registers (FF) & 18,650 & 106,400 & 17.53\% \\
PL Dynamic Logic Power & 0.047~W & --- & --- \\
Total SoC Power & 1.438~W & --- & --- \\
Tile Processing Latency ($128 \times 128$) & 6.33~ms & --- & --- \\
